<template>
  <div class="category-page">
    <div class="page-header">
      <h1 class="page-title">{{ t('categorySettings') }}</h1>
    ...
    <el-dialog v-model="formDlg" :title="editTarget ? t('edit') : t('add')" width="420px" :close-on-click-modal="false">
...
import { getInventoryCategories, createInventoryCategory, updateInventoryCategory, deleteInventoryCategory } from '../api'
import { t } from '../i18n'
...
const cfmForm = async () => {
  ...
  try {
    if (editTarget.value) { await updateInventoryCategory(editTarget.value.id, form.value.name, form.value.description) }
    else { await createInventoryCategory(form.value.name, form.value.description) }
